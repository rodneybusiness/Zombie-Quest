...
class PathCharacter(Character):
    ...
    def update(self, dt: float) -> None:
        self.moving = self.follow_path(dt)
        self.update_animation(dt, self.moving)

    def is_moving(self) -> bool:
        return self.moving

    def distance_to(self, point: Tuple[float, float]) -> float:
        return pygame.Vector2(point).distance_to(self.position)


class Hero(PathCharacter):
    def __init__(self, position: Tuple[float, float]) -> None:
        animations = assets.load_character_animations("Hero", frame_size=(36, 72), frames_per_direction=6)
        super().__init__("Hero", animations, position, speed=70.0)


class Zombie(PathCharacter):
    def __init__(self, position: Tuple[float, float]) -> None:
        animations = assets.load_character_animations("Zombie", frame_size=(34, 70), frames_per_direction=5)
        super().__init__("Zombie", animations, position, speed=42.0)
